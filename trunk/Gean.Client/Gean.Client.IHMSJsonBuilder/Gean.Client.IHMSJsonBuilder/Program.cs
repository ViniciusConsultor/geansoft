...
namespace Gean.Client.IHMSJsonBuilder
{
    public static class Program
    {
        ...
        {
//            Groups = new GroupEnt[20];
//            Operations = new OperationEnt[8];

//            #region 20个营业点,8个业务
//            for (int i = 0; i < 20; i++)//20个营业点
//            {
//                GroupEnt ge = new GroupEnt();
//                ge.id = Guid.NewGuid().ToString("D");
//                ge.name = "name";
//                ge.alias = "alias";
//                Groups[i] = ge;
//            }
//            for (int i = 0; i < 8; i++)//8个业务
//            {
//                OperationEnt oe = new OperationEnt();
//                oe.id = Guid.NewGuid().ToString("D");
//                oe.name = "name";
//                Operations[i] = oe;
//            }
//            #endregion

//            IHMSGroupJsonData dataByJson = new IHMSGroupJsonData(Groups.Length, Operations.Length, 10, 12);

//            Json json = new Json();

//            string stringByJson = json.SerializeObject(dataByJson);
//            File.AppendAllText(Path.Combine(@"F:\My Desktop\", "Data" + DateTime.Now.Ticks + ".txt"), stringByJson);

//            string groupByJson = json.SerializeObject(Groups);
//            File.AppendAllText(Path.Combine(@"F:\My Desktop\", "Group" + DateTime.Now.Ticks + ".txt"), groupByJson);

//            string operationByJson = json.SerializeObject(Operations);
//            File.AppendAllText(Path.Combine(@"F:\My Desktop\", "Operation" + DateTime.Now.Ticks + ".txt"), operationByJson);
//        }
    }

//        public static GroupEnt[] Groups;
//        public static OperationEnt[] Operations;

//    [Serializable]
//    public class IHMSGroupJsonData
//    {
...
//            {
//                this.Staticstics[i] = new Group(i, o, c, w);
//            }
//        }
//    }
...
//        public Group(int index, int o, int c, int w)
//        {
//            this.Id = Program.Groups[index].id;
...
//        {
//            this.Id = Program.Operations[index].id;
...
//            jdata.T[3] = Convert.ToInt32((jdata.T[2] * random.Next(30, 80)) / 100); //有效服务人数
//            jdata.T[4] = jdata.T[2] - jdata.T[3]; //弃票人数
//            jdata.T[0] = (random.Next(5, jdata.T[2])) / 15; //正在等待人数
//            jdata.T[1] = random.Next(0, jdata.T[0] / 3); //正在办理人数

//            //评价人数，未评价，未请评价，非常满意人数，满意人数，一般人数，不满意人数
//            int ev = Convert.ToInt32((jdata.T[3] * random.Next(40, 80)) / 100);
//            jdata.E[0] = ev; //评价人数
//            jdata.E[1] = Convert.ToInt32(((jdata.T[3] - ev) * random.Next(60, 90)) / 100);//未评价
//            jdata.E[2] = jdata.T[3] - jdata.E[0] - jdata.E[1];//未请评价
...
//            jdata.W[0] = Convert.ToInt32((jdata.T[3] * random.Next(15, 20)) / 100);//3--
//            jdata.W[1] = Convert.ToInt32((jdata.T[3] * random.Next(15, 20)) / 100);//3-5
//            jdata.W[2] = Convert.ToInt32((jdata.T[3] * random.Next(15, 20)) / 100);//5-10
//            jdata.W[4] = Convert.ToInt32((jdata.T[3] * random.Next(15, 20)) / 100);//20++
//            jdata.W[3] = jdata.T[3] - jdata.W[0] - jdata.W[1] - jdata.W[2] - jdata.W[4];//10-20分钟
