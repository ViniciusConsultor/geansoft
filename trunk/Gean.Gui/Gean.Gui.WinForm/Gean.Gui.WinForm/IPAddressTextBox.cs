...
namespace Gean.Gui.WinForm
{
    ...
    public class IPAddressTextBox : System.Windows.Forms.TextBox
    {
        ...
        private IPNotation _ipNotation = IPNotation.IPv4Decimal;
        private IPNotation _newIPNotation = IPNotation.IPv4Decimal;
        private bool _bOverwrite = true;
        private bool _bPreventLeave = true;
        private System.Windows.Forms.ErrorProvider error;
        private Regex _regexValidNumbers = new Regex("[0-9]");
        private ArrayList _arlDelimeter = new ArrayList(new char[] { '.' });
        ...
        public override string Text
        {
            ...
            set
            {
                try
                {
                    if (IPAddressTextBox.ValidateIP(value, this._newIPNotation, this._arlDelimeter))
                        base.Text = IPAddressTextBox.MakeValidSpaces(value, this._newIPNotation, this._arlDelimeter);
                }
                catch
                {
        ...
        public bool PreventLeaveAtError
        {
            get
            {
                return this._bPreventLeave;
            }
            set
            {
                if (value != this._bPreventLeave)
                {
                    this._bPreventLeave = value;
        ...
        public IPNotation Notation
        {
            ...
            set
            {
                if (value != this._ipNotation)
                {
                    try
                    {
                        this._newIPNotation = value;
                        this.ChangeNotation(this._ipNotation, this._newIPNotation);
                        this._ipNotation = this._newIPNotation;
                        this.OnNotationChanged(this._newIPNotation);
                    }
        ...
        {
            string sTo = "";
            ArrayList arlFrom = new ArrayList(this.Text.Replace(" ", "").Split((char[])this._arlDelimeter.ToArray(typeof(char))));

            switch (arg_newValue)
            {
                case IPNotation.IPv4Decimal:
                    this._regexValidNumbers = new Regex("[0-9]");
                    this._arlDelimeter = new ArrayList(new char[] { '.' });
                    break;
                case IPNotation.IPv4DecimalCIDR:
                    this._regexValidNumbers = new Regex("[0-9]");
                    this._arlDelimeter = new ArrayList(new char[] { '.', '/' });
                    break;
                case IPNotation.IPv4Binary:
                    this._regexValidNumbers = new Regex("[01]");
                    this._arlDelimeter = new ArrayList(new char[] { '.' });
                    break;
                case IPNotation.IPv4BinaryCIDR:
                    this._regexValidNumbers = new Regex("[01]");
                    ...
                    break;
                case IPNotation.IPv6Hexadecimal:
                    this._regexValidNumbers = new Regex("[0-9a-fA-F]");
                    this._arlDelimeter = new ArrayList(new char[] { ':' });
                    break;
                case IPNotation.IPv6HexadecimalCIDR:
                    this._regexValidNumbers = new Regex("[0-9a-fA-F]");
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (e.Modifiers == Keys.None)
            {
                ...
                {
                    if (this._arlDelimeter.Contains(cText[iPos]))
                        iPos += 1;
        ...
        protected override void OnKeyPress(KeyPressEventArgs e)
        {
            ...
            if (char.IsControl(e.KeyChar) ||
                _regexValidNumbers.IsMatch(e.KeyChar.ToString()))
            {
                e.Handled = false;
            }
            else
        ...
        protected override void OnValidating(CancelEventArgs e)
        {
            ...
            e.Cancel = (!this.ValidateIP() && this._bPreventLeave);
            base.OnValidating(e);
        }
        ...
        {
            string s = "";
            ArrayList arlIP = new ArrayList(this.Text.Replace(" ", "").Split((char[])this._arlDelimeter.ToArray(typeof(char))));
            for (int i = 0; i < arlIP.Count; i++)
            {
                while (arlIP[i].ToString().StartsWith("0"))
                    arlIP[i] = arlIP[i].ToString().Substring(1);
            }
            s = IPAddressTextBox.MakeIP((string[])arlIP.ToArray(typeof(string)), this._ipNotation);
            //in IPv6 Addresses replace 0000: by ::
            if (this._ipNotation == IPNotation.IPv6Hexadecimal ||
                this._ipNotation == IPNotation.IPv6HexadecimalCIDR ||
                this._ipNotation == IPNotation.IPv6Binary ||
                this._ipNotation == IPNotation.IPv6BinaryCIDR)
            {
                while (s.IndexOf(":::") >= 0)
                {
        ...
        {
            if (IPAddressTextBox.ValidateIP(this.Text, this._newIPNotation, this._arlDelimeter))
                return true;
            else
