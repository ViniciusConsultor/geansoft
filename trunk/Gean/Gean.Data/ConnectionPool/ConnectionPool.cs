...
namespace Gean.Data
{
    ...
    public class ConnectionPool : IDisposable
    {
        ...
        //内部对象
        private List<ConnectionStruct> al_All = new List<ConnectionStruct>();//实际连接
        ...
        /// <returns></returns>
        private DbConnection GetConnectionFormPool_Return(object key, ConnectionStruct cs, ConnLevel cl)
        {
            try
            {
                if (cs == null)
        ...
        public void DisposeConnection(object key)
        {
            lock (hs_UseConn)
            {
                ...
                if (_ps == PoolState.Run)
                {
                    ...
                    cs = (ConnectionStruct)hs_UseConn[key];
                    cs.IsRepeat = true;
                    if (cs.Allot == false)
                        if (cs.Enable == true)
                            cs.Allot = true;
        ...
        private ConnectionStruct CreateConnection(string conn, ConnTypeEnum cte)
        {
            ...
            ConnectionStruct cs = new ConnectionStruct(db, cte, DateTime.Now);
            cs.Open();
            return cs;
        }
        /// <summary>
        ...
        private void TestConnStruct(ConnectionStruct cs)
        {
            //此次被分配出去的连接是否在此次之后失效
