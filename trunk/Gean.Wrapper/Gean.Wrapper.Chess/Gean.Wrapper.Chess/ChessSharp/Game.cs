...
//			OpeningBook.BookConvert(Game.PlayerWhite);
		...
		public static void MakeAMove(Enums.Action action, ChessPiece piece, Square square)
		{
			SuspendPondering();
			MakeAMove_Internal(action, piece, square);
			Game.SaveBackup();
			SendBoardPositionChangeEvent();
			CheckIfAutoNextMove();
		}

		private static void MakeAMove_Internal(Enums.Action action, ChessPiece piece, Square square)
		{
			m_movesRedoList.Clear();
			Move move = piece.Move(action, square);
