...
namespace Gean.Wrapper.Sudoku
{
    ...
    public class DoTable
    {
        ...
        public DoCell this[int row, int col]
        {
        ...
        public void Solve()
        {
            ...
            List<DoCell> sudokuCellsToSolve = new List<DoCell>();
            ...
            while (sudokuCellsToSolve.Count > 0)
            {
                ...
                DoCell cell = sudokuCellsToSolve[index];
                ...
                this.OnSudokuStrategyUsed(
                    new DoStrategyUsedEventArgs(DoStrategy.RandomPick, cell));
        ...
        public static int[] Solve(string doString)
        {
            int r = 0;
            int c = 0;
            DoTable dt = new DoTable();
            for (int i = 0; i < 81; i++)
            {
                r = i / 9;
                c = i % 9;
                dt[r, c] = new DoCell(r, c);
            }
            int[] doArray = DoHelper.GetDoIntArrary(doString);
        ...
        private bool SolveCellsWithOnePossibility(List<DoCell> sudokuCellsToSolve)
        {
            bool result = false;
            int i = 0;
            ...
            while (i < sudokuCellsToSolve.Count)
            {
                ...
                {
                    DoCell cell = sudokuCellsToSolve[i];
                    int solution = cell.PickRandomPossibility();
                    if (solution < 0)
                    {
                    ...
                    this.SetValue(cell.Row, cell.Column, solution);
                    this.OnSudokuStrategyUsed(
                        new DoStrategyUsedEventArgs(DoStrategy.UniqueValueForCell, cell));
                    result = true;
                ...
                else
                {
                    for (int value = 0; value < 9; value++)
                    {
                        if (valueMask[value] >= 0)
                        {
                            ...
                            this.OnSudokuStrategyUsed(
                                new DoStrategyUsedEventArgs(DoStrategy.UniqueValueForRow,
                ...
                else
                {
                    for (int value = 0; value < 9; value++)
                    {
                        if (valueMask[value] >= 0)
                        {
                            ...
                            this.OnSudokuStrategyUsed(
                                new DoStrategyUsedEventArgs(DoStrategy.UniqueValueForColumn,
                ...
                else
                {
                    for (int value = 0; value < 9; value++)
                    {
                        if (valueMask[value] >= 0)
                        {
                            ...
                            this.OnSudokuStrategyUsed(
                                new DoStrategyUsedEventArgs(DoStrategy.UniqueValueForSquare,
                                this[rowS, colS]));
                            return true;
                        }
                    }
        ...
        protected virtual void OnSudokuStrategyUsed(DoStrategyUsedEventArgs e)
        {
